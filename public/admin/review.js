...
(() => {
  ...
  function install() {
    ...
    const frontendRecoveryLabel = document.createElement('label'); frontendRecoveryLabel.innerHTML = `随站发布的恢复方案<select name="recoveryProfileId" required><option value="">正在读取恢复方案…</option></select><small>每个独立前台必须绑定一套已启用且已发布的恢复方案；发布后该前台只接收该方案的备用域名和 DNS/TXT 查询线路。</small>`;

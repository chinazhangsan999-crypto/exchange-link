...
(() => {
  ...
  async function loadPublicFrontendProfiles() { try { const data = await api('/api/admin/integrations/cloudflare-public-frontends'), button = document.querySelector('#create-public-frontend'), state = document.querySelector('#create-public-frontend-state'), profileState = document.querySelector('#public-frontend-profile-state'), form = document.querySelector('#public-frontend-profile-form'); const profiles = data.profiles || []; if (form) { form.elements.apiToken.required = profiles.length === 0 && !form.elements.reuseCentralCredential.checked; syncCentralCredentialChoice(); } if (button) button.disabled = profiles.length === 0; if (state) state.textContent = profiles.length ? `已配置 ${profiles.length} 个账号；每个新前台域名都会创建新的独立 Worker。` : '请先配置至少一个公共前台 Cloudflare 账号。'; if (profileState) profileState.textContent = profiles.length ? `已配置：${profiles.map(profile => `${profile.label}（前缀：${profile.workerPrefix}；初始化：${profile.initialized ? '完成' : '待处理'}；已记录 Worker：${(profile.workers || []).length}）`).join('；')}` : '尚未配置公共前台账号。'; } catch (error) { toast(error.message); } }
  ...
  async function createPublicFrontend(event) { event.preventDefault(); const form = event.currentTarget, button = form.querySelector('#create-public-frontend'), hostname = String(form.elements.hostname.value || '').trim(); if (!confirm(`确定创建并启用独立前台 ${hostname} 吗？该域名会部署一个全新的 Worker，并同步两层白名单。\n\n免费版提醒：请确认该根域已在 Cloudflare 控制台手动开启 Bot Fight 模式。`)) return; try { button.disabled = true; button.textContent = '部署独立 Worker 中…'; const result = await api('/api/admin/public-frontends', { method: 'POST', body: JSON.stringify({ hostname }) }); const state = document.querySelector('#create-public-frontend-state'), status = result.health?.healthy ? `已创建独立 Worker ${esc(result.service)}：<a href="${esc(result.url)}" target="_blank" rel="noopener">${esc(result.url)}</a>，健康检查通过。` : `已创建独立 Worker ${esc(result.service)} 与 ${esc(result.url)}，Cloudflare 正在配置证书或路由，请稍后刷新验证。`; if (state) state.innerHTML = `${status}<br><strong>请确认该根域的 Cloudflare 免费版 Bot Fight 模式已经手动开启；同一根域只需开启一次。</strong>`; form.elements.hostname.value = ''; await Promise.all([loadFrontendOrigins(), loadCloudflareCentralAccess(), loadPublicFrontendProfiles()]); alert('独立前台生成流程已完成。\n\n请确认该根域已在 Cloudflare「安全性 → 设置 → 机器人流量」中手动开启 Bot Fight 模式。同一根域只需开启一次。'); toast(result.health?.healthy ? '新独立前台已创建并可访问' : '新独立前台已创建，等待 Cloudflare 生效'); } catch (error) { toast(error.message || '创建新独立前台失败'); } finally { button.disabled = false; button.textContent = '生成独立前台'; } }

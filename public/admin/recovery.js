...
(() => {
  ...
  function renderRoutePlan(plan) {
    ...
    const tierRows = plan.tiers.map(tier => {
      const timeoutText = tier.resolverIds.map(resolverId => `${resolverId} ${Number(tier.resolverTimeouts?.[resolverId] || tier.timeoutMs)} ms`).join(' · ');
      return `<div class="recovery-route-tier" data-complete="${tier.complete ? '1' : '0'}"><span><strong>P${tier.priorityGroup} · ${escapeHtml(tier.label)}</strong><small>${escapeHtml(timeoutText)}</small></span><b>${tier.configured}/${tier.expected}</b><em>${tier.complete ? '完整' : '待补充'}</em></div>`;
    }).join('');
  ...
  async function handleAction(event) {
    ...
    try {
      ...
      if (action === 'test-group-routes') {
        ...
        const statusTone = result.status === 'failed' ? 'error' : result.status === 'degraded' ? 'warning' : '';
        container.innerHTML = `<div class="recovery-result ${statusTone ? `recovery-result-${statusTone}` : ''}"><strong>${escapeHtml(result.group.label)} · ${escapeHtml(result.statusLabel || '三层查询线路检测')}</strong><p>${summary.join('，')}。</p>${graceHint}<div class="recovery-test-tier-list">${tierRows}</div></div>${detailRows}`;
        notify(`${result.statusLabel || '线路检测完成'}：成功 ${result.successful}，传播中 ${result.propagating || 0}，解析器不可用 ${result.resolverUnavailable || 0}`);
        return;
      }

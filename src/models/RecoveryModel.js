...
function listLookupRoutes(owner=1,{enabledOnly=false}={}){return all(`SELECT r.*,d.label AS resolver_label,d.endpoint,d.response_format,d.category AS resolver_category,b.label AS bootstrap_label,b.record_name,b.share_role,b.group_id,b.required_target,b.status AS bootstrap_status FROM recovery_lookup_routes r JOIN recovery_resolvers d ON d.id=r.resolver_id JOIN recovery_bootstrap_records b ON b.id=r.bootstrap_id WHERE r.profile_id=? ${enabledOnly?'AND r.status=1 AND d.enabled=1 AND b.status=1':''} ORDER BY r.priority_group,r.sort_order,r.id`,[profileId(owner)]);}
...
async function applyLookupRoutePlan(plan,owner=1){return withTransaction(async transaction=>{
  for(const id of plan.deleteIds||[]) await transaction.run('DELETE FROM recovery_lookup_routes WHERE id=? AND profile_id=?',[Number(id),profileId(owner)]);
  for(const item of plan.updates||[]) await transaction.run(`UPDATE recovery_lookup_routes SET priority_group=?,timeout_ms=?,sort_order=?,status=1,updated_at=CURRENT_TIMESTAMP WHERE id=? AND profile_id=?`,[item.priorityGroup,item.timeoutMs,item.sortOrder,Number(item.id),profileId(owner)]);
  for(const item of plan.creates||[]) await transaction.run(`INSERT INTO recovery_lookup_routes(profile_id,resolver_id,bootstrap_id,priority_group,timeout_ms,sort_order,status) VALUES(?,?,?,?,?,?,1)`,[profileId(owner),item.resolverId,item.bootstrapId,item.priorityGroup,item.timeoutMs,item.sortOrder]);
  return transaction.all(`SELECT r.*,d.label AS resolver_label,d.endpoint,d.response_format,d.category AS resolver_category,b.label AS bootstrap_label,b.record_name,b.share_role,b.group_id,b.required_target,b.status AS bootstrap_status FROM recovery_lookup_routes r JOIN recovery_resolvers d ON d.id=r.resolver_id JOIN recovery_bootstrap_records b ON b.id=r.bootstrap_id WHERE r.profile_id=? ORDER BY r.priority_group,r.sort_order,r.id`,[profileId(owner)]);
},{durability:'full',label:'apply recovery lookup route plan'});}
...
function listPendingReleases(limit=20){return all("SELECT * FROM recovery_releases WHERE status='pending' ORDER BY generation ASC LIMIT ?",[Math.max(1,Math.min(100,Number(limit)||20))]);}

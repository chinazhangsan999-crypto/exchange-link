...
const { parseHostname, matchesPartnerDomain } = require('../utils/network');
...
async function createPartner(req, res) {
  try {
    ...
    if (!cleanDomain) return fail(res, '无法从网站地址识别域名');
...
async function updatePartner(req, res) {
  try {
    ...
    if (body.url !== undefined) {
      const cleanUrl = normalizeUrl(body.url);
      const cleanDomain = new URL(cleanUrl).hostname.toLowerCase().replace(/^www\./, '');
      if (!cleanDomain) return fail(res, '无法从网站地址识别域名');
      ...
      if (!domain) return fail(res, '来源域名不能为空');
...
async function syncPartnersMatrix(req, res) {
  try {
    ...
    const items = sourceRows.map(row => {
      const url = normalizeMatrixUrl(row['网站地址(URL)'], '网站地址', row.__line);
      const name = row['网站名称'];
    ...
    ensureUnique(items, item => item.url, '网站地址');
    const result = await PartnerModel.syncPartnersFromCsv(items);
    CacheService.clearPublicCache();
    return ok(res, { type: 'partners', ...result }, `友链新增 ${result.inserted} 条，更新 ${result.updated} 条`);
  } catch (error) {

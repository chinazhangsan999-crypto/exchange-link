...
function buildClientAuditRows(events, interactions, pv, thresholds, pageViewsByVisit = []) {
  ...
  return [...groups.values()].map(group => {
    ...
    return {
      ...
      ip_network_type_confidence: String(latest.ip_network_type_confidence || 'unknown'),
      ip_country_code: String(latest.ip_country_code || ''),
      ip_country_name: String(latest.ip_country_name || ''),
      ip_country_name_zh: String(latest.ip_country_name_zh || ''),
      ip_region: String(latest.ip_region || ''),
      ip_region_zh: String(latest.ip_region_zh || ''),
      ip_city: String(latest.ip_city || ''),
      ip_city_zh: String(latest.ip_city_zh || ''),
      ip_timezone: String(latest.ip_timezone || ''),
      ip_timezone_zh: String(latest.ip_timezone_zh || ''),
      ...
      ip_isp_zh: String(latest.ip_isp_zh || ''),
      ...
      ip_special_purpose_zh: String(latest.ip_special_purpose_zh || ''),
      ...
      ip_crawler_operator_zh: String(latest.ip_crawler_operator_zh || ''),
      ip_crawler_type: String(latest.ip_crawler_type || ''),
      ip_crawler_type_zh: String(latest.ip_crawler_type_zh || ''),
      ip_is_private_relay: latest.ip_is_private_relay,
      ip_private_relay_region: String(latest.ip_private_relay_region || ''),
      ip_private_relay_region_zh: String(latest.ip_private_relay_region_zh || ''),
...
async function analyzePartnerClients(partnerId, { page = 1, pageSize = 100, query = '', filter = 'all' } = {}) {
  ...
  const filteredRows = allRows.filter(row => {
    ...
    return [
      row.ip, row.ip_network_type, row.ip_network_type_zh, row.ip_country_name, row.ip_country_name_zh,
      row.ip_region, row.ip_region_zh, row.ip_city, row.ip_city_zh,
      row.ip_asn_org, row.ip_asn_org_zh, row.ip_isp, row.ip_isp_zh, row.ip_confidence, row.ip_confidence_zh,
      row.ip_crawler_operator, row.ip_crawler_operator_zh, row.ip_crawler_type, row.ip_crawler_type_zh,
      row.ip_private_relay_region, row.ip_private_relay_region_zh, row.ip_special_purpose, row.ip_special_purpose_zh,
